...
tox.load = function (file) {
  if (page.exists(tox.options.path, file + '.' + tox.options.ext)) {
    ...
    if (tox.options.debug) console.log('Missing: %s', file + '.' + tox.options.ext);
...
tox.page = function (req, res, next) {
  if (tox.options.exclude.indexOf((typeof req === 'string' ? req : req.params[0])) !== -1) return;
  if (typeof req === 'string' && res === undefined) {
    return tox.load((req.length ? req : tox.options.startPage));
  } else {
